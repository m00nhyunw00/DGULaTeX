...
| 운영 편의 | 루트 npm run dev로 프론트엔드와 백엔드 동시 실행, Yjs는 백엔드 5000 포트의 /yjs 경로에 통합 실행, dev:fresh/stop:dev로 포트 충돌 정리 |
...
- 프론트엔드는 같은 브라우저의 여러 탭이 하나의 로그인 상태를 공유하도록 세션 토큰과 사용자 UUID를 localStorage에 저장합니다.
...
Browser
  ...
  +- Express BackEnd :5000
     ...
     +- Yjs WebSocket /yjs shared document synchronization
     +- MySQL persistence
